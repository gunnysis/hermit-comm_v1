...
2. 입력 항목:
   - **제목**: 필수, 최대 100자.
   - **내용**: 필수, 최대 5000자. **리치 텍스트**(굵게/기울임/밑줄 등) 지원, HTML로 저장되어 본문 보기 시 서식이 유지됩니다.
   - **닉네임(선택)**:
...
> 운영자가 앱 안에서 **초대 코드가 포함된 그룹을 생성**하고, 이 코드를 참여자에게 전달해 준다는 전제를 가집니다.

1. 하단 탭에서 **그룹** 탭(내 그룹 화면)을 엽니다.
2. 화면 상단에 있는 **`초대 코드로 참여` 카드**에서, 운영자로부터 받은 코드를 입력합니다.
3. **참여하기** 버튼을 누르면:
   - 코드가 유효한 경우 → 해당 그룹에 자동으로 참여되고, `내 그룹 목록` 섹션에 새로운 카드가 추가됩니다.
   - 이미 참여 중인 그룹인 경우 → 별도의 에러 없이 \"이미 참여 중인 그룹이에요.\" 라는 안내만 표시됩니다.
   - 코드가 잘못된 경우 → \"존재하지 않는 초대 코드입니다.\" 라는 안내가 표시됩니다.
4. 참여 이후에는 `내 그룹 목록`에서 해당 그룹 카드를 눌러, 그룹 게시판으로 이동해 글을 읽고 작성할 수 있습니다.
...
5. **관리자 페이지 (앱 내)**
   - 앱 관리자(개발자/운영자)는 **하단 탭의 `설정` 화면**에서 우측 하단에 있는 작은 링크를 통해 관리자 페이지에 진입할 수 있습니다.
     - 설정 탭 → 화면 오른쪽 아래의 텍스트 링크 **\"운영자용 관리자 페이지로 이동\"** 을 누르면 관리자 로그인 화면(`/admin/login`)으로 이동합니다.
   - 관리자 여부는 **DB의 `app_admin` 테이블 조회**로만 판단합니다. 환경 변수는 사용하지 않습니다.
   - 관리자 **로그인**: Supabase Auth에서 **이메일/비밀번호**로 생성한 사용자만 관리자로 등록할 수 있습니다. Supabase 대시보드에서 해당 사용자를 만든 뒤 `INSERT INTO app_admin (user_id) VALUES ('해당 auth.users.id');` 로 등록하고, 앱에서는 위 링크 또는 딥링크로 `/admin/login` 화면에 진입한 후 이메일/비밀번호로 로그인하면 관리자 페이지에 접근할 수 있습니다.
   - 관리자 페이지에서 할 수 있는 일:
     - **그룹 게시판 생성**: 그룹명, 초대 코드, 설명(선택)을 입력해 한 번에 그룹 + 기본 보드 + 본인을 owner로 등록합니다.
     - **내가 만든 그룹 목록**: 본인이 owner인 그룹 목록을 보고, 각 그룹의 초대 코드를 **공유** 버튼으로 공유할 수 있습니다.
   - 로그아웃 시 익명 세션으로 돌아가며, 일반 사용자처럼 앱을 계속 사용할 수 있습니다.
