...
5. **관리자 페이지 (앱 내)**
   - 앱 관리자(개발자/운영자)는 **내 그룹** 화면에서 **관리자** 버튼을 눌러 관리자 페이지에 진입할 수 있습니다.
   - 관리자 여부는 **DB의 `app_admin` 테이블 조회**로만 판단합니다. 환경 변수는 사용하지 않습니다.
   - 관리자 **로그인**: Supabase Auth에서 **이메일/비밀번호**로 생성한 사용자만 관리자로 등록할 수 있습니다. Supabase 대시보드에서 해당 사용자를 만든 뒤 `INSERT INTO app_admin (user_id) VALUES ('해당 auth.users.id');` 로 등록하고, 앱에서는 **관리자** → **관리자 로그인** 화면에서 이메일/비밀번호로 로그인하면 관리자 페이지에 접근할 수 있습니다.
   - 관리자 페이지에서 할 수 있는 일:
