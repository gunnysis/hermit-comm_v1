...
export const EMOTION_COLOR_MAP: Record<
  string,
  {
    family: string;
...
export const EMPTY_STATE_MESSAGES = {
  ...
  emotion_filter: {
    title: '이 감정의 이야기가 아직 없어요',
    description: '비슷한 마음을 느끼고 있다면,\n용기 내어 이야기해 주세요.',
  },
  comments: {
    title: '아직 댓글이 없어요',
    description: '따뜻한 한마디가\n누군가에게 큰 위로가 될 수 있어요.',
  },
  recommendations: {
...
export function validateGroupInput(input: {
  ...
  if (name.length > ADMIN_CONSTANTS.GROUP_NAME_MAX_LENGTH)
    return `그룹 이름은 ${ADMIN_CONSTANTS.GROUP_NAME_MAX_LENGTH}자 이내로 입력해주세요.`;

  if (input.inviteCode?.trim()) {
    const code = input.inviteCode.trim();
    if (code.length < ADMIN_CONSTANTS.INVITE_CODE_MIN_LENGTH)
      return `초대 코드는 ${ADMIN_CONSTANTS.INVITE_CODE_MIN_LENGTH}자 이상이어야 합니다.`;
    if (code.length > ADMIN_CONSTANTS.INVITE_CODE_MAX_LENGTH)
      return `초대 코드는 ${ADMIN_CONSTANTS.INVITE_CODE_MAX_LENGTH}자 이내로 입력해주세요.`;
  }
